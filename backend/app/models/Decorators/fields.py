...
def fields_get(methods="*",out="fields",tpe="json"):
        ...
        def wrapper(*args, **kwargs):
            ...
            if methods == "*" or request.method in methods:
                if request.method == "GET":
                    ...
                    data = request.get_json(force=True) or request.get_json() if tpe =="json" else request.form.to_dict()
        ...
        def wrapper(*args, **kwargs):
            ...
            if isinstance(lista,dict):
                for k,v in lista.items():
                    if (v != str and fields[k] =='') or fields[k].lower() == 'null':
                        fields[k] = None
                    if v == int and isinstance(fields[k],str) and fields[k].isnumeric():
                        fields[k] = int(fields[k])
                    if v == bool and isinstance(fields[k],str):
                        val = fields[k].lower()
                        fields[k] = eval(val.capitalize()) if fields[k] in ['true','false'] else bool(fields[k])
                    if v == float and isinstance(fields[k],str) and fields[k].replace(".","").isnumeric():
                        fields[k] = float(fields[k])
                    if v == list and isinstance(fields[k],str):
                        fields[k] = eval(fields[k])
...
def fields_capiitalize(lista): #fields_notEmpty(['key1','key2'])        => string|list
    def decorator(function):
        @wraps(function)
        def wrapper(*args, **kwargs):
